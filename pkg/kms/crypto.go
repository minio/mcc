...
// EncryptBytes encrypts the plaintext with a key managed by
// The context is bound to the returned ciphertext.
//
// The same context must be provided when decrypting the
// ciphertext.
func EncryptBytes(KMS KMS, plaintext []byte, context Context) ([]byte, error) {
...
// DecryptBytes decrypts the ciphertext using a key managed by the
// The same context that have been used during encryption must be
// provided.
func DecryptBytes(KMS KMS, ciphertext []byte, context Context) ([]byte, error) {
...
// Encrypt encrypts the plaintext with a key managed by
// The context is bound to the returned ciphertext.
//
// The same context must be provided when decrypting the
// ciphertext.
func Encrypt(KMS KMS, plaintext io.Reader, context Context) (io.Reader, error) {
...
// Decrypt decrypts the ciphertext using a key managed by the
// The same context that have been used during encryption must be
// provided.
func Decrypt(KMS KMS, ciphertext io.Reader, context Context) (io.Reader, error) {
	const (
		MaxMetadataSize = 1 << 20 // max. size of the metadata
		Version         = 1
	)
